...
const APP_USER_MODEL_ID = 'com.custom.messenger';
const TOAST_ACTIVATOR_CLSID = '{025A3CBC-B235-49B0-8BAE-D313CBED0DDD}';
...
const MAX_NOTIFICATION_THREAD_ID_CHARS = 256;
...
function isAllowedPermissionRequest({ permission, requestingUrl, isMainFrame, mediaTypes }) {
  ...
  switch (permission) {
    case 'notifications':
      // Facebook's origin also covers friend requests, group activity, Reels,
      // Marketplace, and other non-message products. Block its Web
      // Notification API completely; the main process emits app-owned toasts
      // only after the Messenger conversation tracker confirms a new message.
      return false;
...
function sanitizeNotificationText(value, maxCharacters) {
  if (typeof value !== 'string' || !Number.isSafeInteger(maxCharacters) || maxCharacters < 1) {
    return null;
  }
  const text = value
    .replace(/[\u0000-\u001f\u007f-\u009f\u202a-\u202e\u2066-\u2069]/g, ' ')
...
function validateMessageNotificationPayload(payload) {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) return null;

  const threadId = typeof payload.threadId === 'string' ? payload.threadId.trim() : '';
  if (!threadId
    || Array.from(threadId).length > MAX_NOTIFICATION_THREAD_ID_CHARS
    || /[\u0000-\u0020\u007f\\/?#]/u.test(threadId)
    || typeof payload.encrypted !== 'boolean') {
    return null;
  }

  const title = sanitizeNotificationText(payload.title, MAX_NOTIFICATION_TITLE_CHARS);
  const body = sanitizeNotificationText(payload.body, MAX_NOTIFICATION_BODY_CHARS);
...
function validateUnreadStatePayload(payload) {
  ...
  const message = payload.message === null || payload.message === undefined
    ? null
    : validateMessageNotificationPayload(payload.message);

  // A sound/toast is allowed only when the same IPC carries valid, bounded
  // message metadata. Badge-only and malformed events stay silent.
  const notify = payload.notify && message !== null;
  return {
    count: payload.count,
    notify,
